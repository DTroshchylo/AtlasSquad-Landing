...
export default class StringScroll {
  ...
  private createStringProgressObject(el: any) {
    ...
    let p1 = data.top ,
      p2 = p1 - this.wH,
      p3 = data.top + data.height,
      p4 = p3 - this.wH

    if (sElPos == tS && sScrPos == tS) {
      data.startPos = p1 - data.oTop
    } else if (sElPos == tS && sScrPos == bS) {
      data.startPos = p2 - data.oTop
    } else if (sElPos == bS && sScrPos == tS) {
      data.startPos = p3 - data.oTop
    } else if (sElPos == bS && sScrPos == bS) {
      data.startPos = p4 - data.oTop
    }


    if (eElPos == tS && eScrPos == tS) {
      data.endPos = p1 + data.oBottom
    } else if (eElPos == tS && eScrPos == bS) {
      data.endPos = p2 + data.oBottom
    } else if (eElPos == bS && eScrPos == tS) {
      data.endPos = p3 + data.oBottom
    } else if (eElPos == bS && eScrPos == bS) {
      data.endPos = p4 + data.oBottom
    }
  ...
  public cleanUpParallax() {
    this.parallO = new Map<string, any>()
    this.actParallO = new Map<string, any>()
    this.actParallOA = new Array<any>()
  }
